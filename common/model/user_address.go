...
type UserAddress struct {
	...
	IsDefault int64     `gorm:"column:is_default;type:tinyint UNSIGNED;comment:是否默认：0-默认，1-不默认的;not null;default:0;" json:"is_default"` // 是否默认：0-默认，1-不默认的
...
func (a *UserAddress) UpdatedAddress(uid int64) error {
	return global.DB.Debug().Table("user_address").Where("uid=?", uid).Updates(&a).Error
}
