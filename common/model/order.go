...
=======
	...
	TotalPostage           float64 `gorm:"column:total_postage;type:decimal(8, 2) UNSIGNED;comment:邮费;not null;default:0.00;" json:"total_postage"`                          // 邮费
	PayPrice               float64 `gorm:"column:pay_price;type:decimal(8, 2) UNSIGNED;comment:实际支付金额;not null;default:0.00;" json:"pay_price"`                              // 实际支付金额
	PayPostage             float64 `gorm:"column:pay_postage;type:decimal(8, 2) UNSIGNED;comment:支付邮费;not null;default:0.00;" json:"pay_postage"`                            // 支付邮费
	...
	Paid                   uint32  `gorm:"column:paid;type:tinyint UNSIGNED;comment:支付状态（0：未支付；1：已支付）;not null;default:0;" json:"paid"`                                      // 支付状态
	...
	Status                 uint32  `gorm:"column:status;type:tinyint(1);comment:订单状态（-1 : 申请退款 -2 : 退货成功 0：待发货；1：待收货；2：已收货；3：待评价；-1：已退款）;not null;default:0;" json:"status"` // 订单状态（-1 : 申请退款 -2 : 退货成功 0：待发货；1：待收货；2：已收货；3：待评价；-1：已退款）
	RefundStatus           uint32  `gorm:"column:refund_status;type:tinyint UNSIGNED;comment:0 未退款 1 申请中 2 已退款;not null;default:0;" json:"refund_status"`                    // 0 未退款 1 申请中 2 已退款
	RefundReasonWapImg     string  `gorm:"column:refund_reason_wap_img;type:varchar(255);comment:退款图片;default:NULL;" json:"refund_reason_wap_img"`                           // 退款图片
	RefundReasonWapExplain string  `gorm:"column:refund_reason_wap_explain;type:varchar(255);comment:退款用户说明;default:NULL;" json:"refund_reason_wap_explain"`                 // 退款用户说明
	...
	RefundReasonWap        string  `gorm:"column:refund_reason_wap;type:varchar(255);comment:前台退款原因;default:NULL;" json:"refund_reason_wap"`                                 // 前台退款原因
	RefundReason           string  `gorm:"column:refund_reason;type:varchar(255);comment:不退款的理由;default:NULL;" json:"refund_reason"`                                         // 不退款的理由
	RefundPrice            float64 `gorm:"column:refund_price;type:decimal(8, 2) UNSIGNED;comment:退款金额;not null;default:0.00;" json:"refund_price"`                          // 退款金额
	DeliveryName           string  `gorm:"column:delivery_name;type:varchar(64);comment:快递名称/送货人姓名;default:NULL;" json:"delivery_name"`                                      // 快递名称/送货人姓名
	DeliveryType           string  `gorm:"column:delivery_type;type:varchar(32);comment:发货类型;default:NULL;" json:"delivery_type"`                                            // 发货类型
	DeliveryId             string  `gorm:"column:delivery_id;type:varchar(64);comment:快递单号/手机号;default:NULL;" json:"delivery_id"`                                            // 快递单号/手机号
	GainIntegral           float64 `gorm:"column:gain_integral;type:decimal(8, 2) UNSIGNED;comment:消费赚取积分;not null;default:0.00;" json:"gain_integral"`                      // 消费赚取积分
	UseIntegral            float64 `gorm:"column:use_integral;type:decimal(8, 2) UNSIGNED;comment:使用积分;not null;default:0.00;" json:"use_integral"`                          // 使用积分
	BackIntegral           float64 `gorm:"column:back_integral;type:decimal(8, 2) UNSIGNED;comment:给用户退了多少积分;default:NULL;" json:"back_integral"`                            // 给用户退了多少积分
	Mark                   string  `gorm:"column:mark;type:varchar(512);comment:备注;not null;" json:"mark"`                                                                   // 备注
	IsDel                  uint32  `gorm:"column:is_del;type:tinyint UNSIGNED;comment:是否删除;not null;default:0;" json:"is_del"`                                               // 是否删除
	...
	IsMerCheck             uint32  `gorm:"column:is_mer_check;type:tinyint UNSIGNED;not null;default:0;" json:"is_mer_check"`
	...
	StoreId                uint32  `gorm:"column:store_id;type:int;comment:门店id;not null;default:0;" json:"store_id"`                               // 门店id
	ShippingType           uint32  `gorm:"column:shipping_type;type:tinyint(1);comment:配送方式 1=快递 ，2=门店自提;not null;default:1;" json:"shipping_type"` // 配送方式 1=快递 ，2=门店自提
	ClerkId                uint32  `gorm:"column:clerk_id;type:int;comment:店员id;not null;default:0;" json:"clerk_id"`                               // 店员id
	IsChannel              uint32  `gorm:"column:is_channel;type:tinyint UNSIGNED;comment:支付渠道(0微信公众号1微信小程序);default:0;" json:"is_channel"`         // 支付渠道(0微信公众号1微信小程序)
	IsRemind               uint32  `gorm:"column:is_remind;type:tinyint UNSIGNED;comment:消息提醒;default:0;" json:"is_remind"`                         // 消息提醒
	IsSystemDel            uint32  `gorm:"column:is_system_del;type:tinyint(1);comment:后台是否删除;default:0;" json:"is_system_del"`                     // 后台是否删除
