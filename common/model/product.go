...
type Product struct {
	Id           int32   `gorm:"column:id;type:mediumint;comment:商品id;primaryKey;not null;" json:"id"`                                     // 商品id
	MerId        uint32  `gorm:"column:mer_id;type:int UNSIGNED;comment:商户Id(0为总后台管理员创建,不为0的时候是商户后台创建);not null;default:0;" json:"mer_id"` // 商户Id(0为总后台管理员创建,不为0的时候是商户后台创建)
	...
	Sales        uint32  `gorm:"column:sales;type:mediumint UNSIGNED;comment:销量;not null;default:0;" json:"sales"`                         // 销量
	Stock        uint32  `gorm:"column:stock;type:mediumint UNSIGNED;comment:库存;not null;default:0;" json:"stock"`                         // 库存
	IsShow       int8    `gorm:"column:is_show;type:tinyint(1);comment:状态（0：未上架，1：上架）;not null;default:1;" json:"is_show"`                 // 状态（0：未上架，1：上架）
	IsHot        int8    `gorm:"column:is_hot;type:tinyint(1);comment:是否热卖;not null;default:0;" json:"is_hot"`                             // 是否热卖
	IsBenefit    int8    `gorm:"column:is_benefit;type:tinyint(1);comment:是否优惠;not null;default:0;" json:"is_benefit"`                     // 是否优惠
	IsBest       int8    `gorm:"column:is_best;type:tinyint(1);comment:是否精品;not null;default:0;" json:"is_best"`                           // 是否精品
	IsNew        int8    `gorm:"column:is_new;type:tinyint(1);comment:是否新品;not null;default:0;" json:"is_new"`                             // 是否新品
	AddTime      uint32  `gorm:"column:add_time;type:int UNSIGNED;comment:添加时间;not null;" json:"add_time"`                                 // 添加时间
	IsPostage    uint8   `gorm:"column:is_postage;type:tinyint UNSIGNED;comment:是否包邮;not null;default:0;" json:"is_postage"`               // 是否包邮
	IsDel        uint8   `gorm:"column:is_del;type:tinyint UNSIGNED;comment:是否删除;not null;default:0;" json:"is_del"`                       // 是否删除
	MerUse       uint8   `gorm:"column:mer_use;type:tinyint UNSIGNED;comment:商户是否代理 0不可代理1可代理;not null;default:0;" json:"mer_use"`         // 商户是否代理 0不可代理1可代理
	...
	IsSeckill    uint8   `gorm:"column:is_seckill;type:tinyint UNSIGNED;comment:秒杀状态 0 未开启 1已开启;not null;default:0;" json:"is_seckill"`    // 秒杀状态 0 未开启 1已开启
	IsBargain    uint8   `gorm:"column:is_bargain;type:tinyint UNSIGNED;comment:砍价状态 0未开启 1开启;default:NULL;" json:"is_bargain"`            // 砍价状态 0未开启 1开启
	IsGood       int8    `gorm:"column:is_good;type:tinyint(1);comment:是否优品推荐;not null;default:0;" json:"is_good"`                         // 是否优品推荐
	IsSub        int8    `gorm:"column:is_sub;type:tinyint(1);comment:是否单独分佣;not null;default:0;" json:"is_sub"`                           // 是否单独分佣
	Ficti        int32   `gorm:"column:ficti;type:mediumint;comment:虚拟销量;default:100;" json:"ficti"`                                       // 虚拟销量
	...
	TempId       int32   `gorm:"column:temp_id;type:int;comment:运费模板ID;not null;default:1;" json:"temp_id"`                                // 运费模板ID
	SpecType     int8    `gorm:"column:spec_type;type:tinyint(1);comment:规格 0单 1多;not null;default:0;" json:"spec_type"`                   // 规格 0单 1多
